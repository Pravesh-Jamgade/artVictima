      ...
      Core::mem_origin_t mem_origin)
{
   ...
   if (metadata_request && block_type == CacheBlockInfo::block_type_t::PAGE_TABLE)
   {
      ...
      {
         ScopedLock sl(getLock());
         if (m_pending_pte_prefetch.valid && m_pending_pte_prefetch.pd_line == ca_address)
         {
      ...
      if (pending.valid)
      {
         SubsecondTime pd_done = t_end_final;
         SubsecondTime prefetch_issue = pd_done;
         ...
         auto issue_prefetch = [&](CacheCntlr* target_cache)
         {
            ...
            {
               PrefetchResult res = target_cache->issuePrefetchFromQueue(eip, prefetch_issue);
