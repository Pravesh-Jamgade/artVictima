   ...
   , m_write_misses(0)
{
   ...
   registerStatsMetric("nuca-cache", m_core_id, "reads-metadata", &m_metadata_reads);
   registerStatsMetric("nuca-cache", m_core_id, "writes-metadata", &m_metadata_writes);
   registerStatsMetric("nuca-cache", m_core_id, "read-misses-metadata", &m_metadata_read_misses);
   registerStatsMetric("nuca-cache", m_core_id, "write-misses-metadata", &m_metadata_write_misses);
}
...
NucaCache::read(IntPtr address, Byte* data_buf, SubsecondTime now, ShmemPerf *perf, bool count,bool is_metadata)
{
   ...
   if (block_info)
   {
      ...
      //nuca_stats.metadata_load_hits[type]++;
   }
   else
   {
      if (count) {
         if(is_metadata){
            ++m_metadata_read_misses;
         }
         else{
            ++m_read_misses;
         }
      ...
      //nuca_stats.metadata_load_misses[type]++;
...
{
   //std::cout<<"Nuca Write:"<<is_metadata<<"\n";
   HitWhere::where_t hit_where = HitWhere::MISS;
...
void NucaCache::markTranslationMetadata(IntPtr address, CacheBlockInfo::block_type_t blocktype){
   ...
   for (int i=0; i < m_cache->getCacheSet(set_index)->getAssociativity(); i++){
