...
namespace ParametricDramDirectoryMSI{
    ...
    PageTableWalkerRadix::~PageTableWalkerRadix(){
        ...
        if(fp){
            fprintf(fp, "proposed_STLB_miss_CPI,%.4f\n", cpi_on_stlb_miss);
            fprintf(fp, "proposed_STLB_miss_avg_stall_cycles,%.2f\n", avg_stall_cycles);
            fprintf(fp, "proposed_STLB_miss_walks,%" PRIu64 "\n", walks);
            if(psc_accesses > 0){
                double psc_miss_rate = (static_cast<double>(psc_misses) / static_cast<double>(psc_accesses)) * 100.0;
                fprintf(fp, "proposed_PSC_miss_rate_pct,%.2f\n", psc_miss_rate);
                fprintf(fp, "proposed_PSC_misses,%" PRIu64 "\n", psc_misses);
                fprintf(fp, "proposed_PSC_accesses,%" PRIu64 "\n", psc_accesses);
                for (int lvl = 0; lvl < stats_radix.number_of_levels; ++lvl){
                    fprintf(fp, "proposed_PSC_L%d_hits,%" PRIu64 "\n", lvl + 1, psc_hits_per_level[lvl]);
                    fprintf(fp, "proposed_PSC_L%d_misses,%" PRIu64 "\n", lvl + 1, psc_misses_per_level[lvl]);
                    String label = String(("proposed_PSC_L" + std::to_string(lvl + 1) + "_miss_latency_cycles").c_str());
                    psc_miss_latency_histograms[lvl].printCsv(fp, label.c_str());
                    ...
                    if(!hitwhere_entries.empty()){
                        ...
                        printCsvLabeledCounts(fp, hitwhere_label.c_str(), hitwhere_entries);
                    }
                }
            }
            if(!traversal_path_counts.empty()){
                ...
                printCsvLabeledCounts(fp, "proposed_PTW_traversal_paths", path_entries);
            }
            stlb_miss_latency_histogram.printCsv(fp, "proposed_TLB_miss_service_latency_cycles");
            ...
            if(total_ns > 0){
                ...
                fprintf(fp, "proposed_PTB_latency_share_pct,%.2f\n", ptb_share);
            }
            fclose(fp);
        }
    }
